    ...
    <div class="panel-produk">
        ...
        <div class="filter-tabs">
            ...
            @foreach($kategoris as $kat)
                <div class="filter-tab" data-kat="{{ $kat }}">{{ $kat }}</div>
            @endforeach
        </div>
        <div class="produk-grid" id="produkGrid">
            @foreach($products as $product)
            <div class="produk-card {{ $product->stok == 0 ? 'habis' : '' }}"
                 data-id="{{ $product->id }}"
                 data-nama="{{ $product->nama }}"
                 data-harga="{{ $product->harga }}"
                 data-stok="{{ $product->stok }}"
                 data-kode="{{ $product->kode }}"
                 data-kat="{{ $product->kategori }}"
                 onclick="tambahKeCart(this)">
                <div class="kat">{{ $product->kategori }}</div>
                <div class="nama">{{ $product->nama }}</div>
                <div class="harga">Rp {{ number_format($product->harga, 0, ',', '.') }}</div>
                <div class="stok {{ $product->stok <= 10 && $product->stok > 0 ? 'low' : '' }}">
                    @if($product->stok == 0)
                        <i class="fa fa-ban"></i> Habis
                    @elseif($product->stok <= 10)
                        <i class="fa fa-triangle-exclamation"></i> Stok: {{ $product->stok }}
                    @else
                        <i class="fa fa-check-circle" style="color:var(--green)"></i> Stok: {{ $product->stok }}
                    @endif
                </div>
...
async function prosesTransaksi() {
    ...
    try {
        const res = await fetch('{{ route("kasir.proses") }}', {
            method: 'POST',
            headers: {
...
function tampilModalSukses(data) {
    ...
    document.getElementById('btnCetakStruk').href = '{{ url("/kasir") }}/' + data.transaction_id + '/struk';
